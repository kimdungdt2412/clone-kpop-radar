...
import { selectSiteArtist } from './SiteArtistSlice';
import { useLazyGetDailyDataQuery, useLazyGetWeeklyDataQuery, useLazyGetMonthlyDataQuery, useGetStartDayQuery, useGetWeekListQuery, useGetMonthListQuery, useGetDailyDataQuery, useGetWeeklyDataQuery, useGetMonthlyDataQuery } from '../../app/services/SiteArtist';

export default function SiteArtist({ isScrollDown, siteId }) {
    const [searchParams] = useSearchParams()
    let { type = "growth", date = "daily", gender = "all", page = "1", day = "", weekId = "", month = "", year = "" } = Object.fromEntries([...searchParams])

    const sideArtitstData = useSelector(selectSiteArtist);
    ...
    const payload = {
        siteId,
        orderCountInPage: sideArtitstData.orderCountInPage,
        lastOrderNo: (isValidNumber(page) ? (Number(page) - 1) : 0) * sideArtitstData.orderCountInPage,
    ...
    const getData = () => {
        switch (date) {
            case "daily":
                return sideArtitstData.dailyData

            case "weekly":
                return sideArtitstData.weeklyData

            case "monthly":
                return sideArtitstData.monthlyData
        ...
        <React.Fragment>
            ...
            {!isScrollDown && (
                ...
                <div className="board-date relative min-h-[17px] mt-[10px] ml-[43px] pr-[15px] max-w-[1100px] lg:mx-auto">
                    <BoardDate data={sideArtitstData} date={date} day={day} weekId={weekId} year={year} month={month} />
         ...
         transition-all duration-300 pb-[30px] overflow-hidden">
                ...
                {(getDaily.isFetching || getWeekly.isFetching || getMonthly.isFetching) ? (
                    ...
                    <div id="main-board" className='relative'>
                        ...
                        <TablePagination total={sideArtitstData.totalCount[date]} orderCountInPage={sideArtitstData.orderCountInPage} currentPage={Number(page)} searchParams={searchParams} />
                    </div>
                )}
