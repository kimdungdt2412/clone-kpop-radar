...
import { useGetStartDayQuery, useGetWeekListQuery,useGetMonthListQuery, useLazyGetDailyDataQuery, useLazyGetMonthlyDataQuery, useLazyGetWeeklyDataQuery, useGetDailyDataQuery, useGetWeeklyDataQuery, useGetMonthlyDataQuery } from '../../app/services/Fancafe';
import { selectFancafe } from './FancafeSlice';
import SortByGender from '../../components/Sort/SortByGender';

export default function Fancafe({ isScrollDown }) {
  const [searchParams] = useSearchParams()
  let { type = "growth", date = "daily", gender = "all", page = "1", day = "", weekId = "", month = "", year = "" } = Object.fromEntries([...searchParams])

  const fancafeData = useSelector(selectFancafe);
  ...
  const payload = {
    orderCountInPage: fancafeData.orderCountInPage,
    lastOrderNo: (isValidNumber(page) ? (Number(page) - 1) : 0) * fancafeData.orderCountInPage,
  ...
  const getData = () => {
    switch (date) {
      case "daily":
        return fancafeData.dailyData

      case "weekly":
        return fancafeData.weeklyData

      case "monthly":
        return fancafeData.monthlyData
    ...
    <React.Fragment>
      ...
      {!isScrollDown && (
        ...
        <div className="board-date relative min-h-[17px] mt-[10px] ml-[43px] pr-[15px] max-w-[1100px] lg:mx-auto">
          <BoardDate data={fancafeData} date={date} day={day} weekId={weekId} year={year} month={month} />
      ...
      <div className="board-content max-w-[1100px] my-0 mx-auto bg-white
        ...
        {(getDaily.isFetching || getWeekly.isFetching || getMonthly.isFetching) ? (
          ...
          <div id="main-board" className='relative'>
            ...
            <TablePagination total={fancafeData.totalCount[date]} orderCountInPage={fancafeData.orderCountInPage} currentPage={Number(page)} searchParams={searchParams} />
          </div>
        )}
